...
except ImportError:  # BlackBox QC plots do not require GeoPandas.
    gpd = None
...
except ImportError:  # BlackBox QC plots do not require coordinate transforms.
    Transformer = None


class ProjectDbError(RuntimeError):
    """Raised when a required project database table is unavailable."""


def _bbox_gnss_qc_page_worker(args):
    (
        db_path,
        common,
        line,
    ...
    g = DSRLineGraphicsMatplotlib(db_path)
    _, out_path = g.plot_bbox_gnss_qc_timeseries(
        bbdata=common["bbdata"],
        dsr_df=common["dsr_df"],
        title=f"Line {line} GNSS QC | Page {page_no} of {total_pages}",
        gnss1_label=common.get("gnss1_label"),
        gnss2_label=common.get("gnss2_label"),
    ...
    return page_no, str(out_path)


def _bbox_motion_metric_page_worker(args):
    (
        db_path,
        common,
        line,
    ...
    g = DSRLineGraphicsMatplotlib(db_path)
    _, out_path = g.plot_bbox_motion_qc_timeseries(
        bbdata=common["bbdata"],
        dsr_df=common["dsr_df"],
        metric=metric,
        title=f"Line {line} {metric} QC | Page {page_no} of {total_pages}",
        vessel_label=common.get("vessel_label"),
        rov1_label=common.get("rov1_label"),
        rov2_label=common.get("rov2_label"),
        time_start=cur_start,
        time_end=cur_end,
        station_label_step=1,
        station_label_fontsize=7,
        ...
        save_dir=save_dir,
        file_name=file_name,
        suffix=f"bbox_{str(metric).lower()}_qc",
        ext=ext,
        dpi=dpi,
        is_show=False,
        close=True,
...
def _bbox_motion_combined_page_worker(args):
    (
        db_path,
        common,
        line,
    ...
    g = DSRLineGraphicsMatplotlib(db_path)
    _, out_path = g.plot_bbox_motion_qc_combined_timeseries(
        bbdata=common["bbdata"],
        dsr_df=common["dsr_df"],
...
class DSRLineGraphicsMatplotlib:
    ...
    def _transform_xy_dataframe(
        ...
        if mask.any():
            if Transformer is None:
                raise ImportError("pyproj is required only for coordinate transformation map layers.")
    ...
    def add_project_shapes_layers_matplotlib(
        ...
        for r in rows:
            ...
            if gpd is None:
                raise ImportError("GeoPandas is required only for shapefile map layers.")
    ...
    def plot_whole_line_node_pages_fast(
        ...
        """
        Fast paged node maps for one line.

        If `common` is supplied, reuse the already loaded shared BBox dataset
        produced by `get_bbox_common_dataset_for_line()` so BB/DSR data is not
        read again.
        """
        ...
        if common is not None:
            df = common.get("dsr_df")
            bbdata = common.get("bbdata")
            cfg_row = common.get("cfg_row")
        else:
            df = self.read_dsr_for_line(line)
            bbdata, cfg_row, start_time, end_time = self.load_bbdata_for_dsr_line(
                line=line,
    ...
    def _prepare_bbox_common_dataset(
            self,
            bbdata,
            dsr_df=None,
            cfg_row=None,
        ...
        """
        Prepare one common cleaned dataset for all BBox plots.

        IMPORTANT:
        - GNSS / motion paged plots need lightweight time-series columns
        - node map plots need full DSR coordinate columns and BB map-track columns

        So do NOT trim away coordinate columns required by node-map cache.
        """

        bb = bbdata.copy() if bbdata is not None else pd.DataFrame()
        ...
        if not bb.empty:
            ...
            # Normalize coordinate field names used by different BlackBox
            # configurations/export versions into one canonical set.
            normalized_cols = {
                "".join(ch for ch in str(col).lower() if ch.isalnum()): col
                for col in bb.columns
            }
            coordinate_aliases = {
                "GNSS1_Easting": ("GNSS1_Easting", "GNSS1Easting", "GNSS1_E", "GNSS1East", "GNSS1_X"),
                "GNSS1_Northing": ("GNSS1_Northing", "GNSS1Northing", "GNSS1_N", "GNSS1North", "GNSS1_Y"),
                "GNSS2_Easting": ("GNSS2_Easting", "GNSS2Easting", "GNSS2_E", "GNSS2East", "GNSS2_X"),
                "GNSS2_Northing": ("GNSS2_Northing", "GNSS2Northing", "GNSS2_N", "GNSS2North", "GNSS2_Y"),
            }
            for canonical, aliases in coordinate_aliases.items():
                if canonical in bb.columns:
                    continue
                for alias in aliases:
                    source = normalized_cols.get("".join(ch for ch in alias.lower() if ch.isalnum()))
                    if source is not None:
                        bb[canonical] = bb[source]
                        break

            # numeric cleanup for GNSS / motion
            numeric_cols = [
                "GNSS1_NOS", "GNSS1_DiffAge", "GNSS1_HDOP",
                "GNSS2_NOS", "GNSS2_DiffAge", "GNSS2_HDOP",
                "GNSS1_Easting", "GNSS1_Northing",
                "GNSS2_Easting", "GNSS2_Northing",
                "VesselSOG", "ROV1_SOG", "ROV2_SOG",
                "VesselCOG", "ROV1_COG", "ROV2_COG",
                "VesselHDG", "ROV1_HDG", "ROV2_HDG",
                "Vessel_MRU1_HDG", "Vessel_MRU1_PITCH", "Vessel_MRU1_ROLL",
                "Vessel_MRU2_HDG", "Vessel_MRU2_PITCH", "Vessel_MRU2_ROLL",

                # map / node-track columns
                "VesselEasting", "VesselNorthing",
            ...
            for c in ["GNSS1_NOS", "GNSS2_NOS"]:
                if c in bb.columns:
                    bb.loc[bb[c] < 0, c] = np.nan

            for c in ["GNSS1_DiffAge", "GNSS2_DiffAge"]:
                if c in bb.columns:
                    bb.loc[(bb[c].notna()) & ((bb[c] < 0) | (bb[c] > 9999)), c] = np.nan

            for c in ["GNSS1_HDOP", "GNSS2_HDOP"]:
                if c in bb.columns:
                    bb.loc[(bb[c].notna()) & (bb[c] < 0), c] = np.nan

            for c in ["VesselSOG", "ROV1_SOG", "ROV2_SOG"]:
                if c in bb.columns:
                    bb.loc[(bb[c].notna()) & ((bb[c] < 0) | (bb[c] > 25)), c] = np.nan

            for c in ["VesselCOG", "ROV1_COG", "ROV2_COG", "VesselHDG", "ROV1_HDG", "ROV2_HDG"]:
                if c in bb.columns:
                    bb.loc[(bb[c].notna()) & ((bb[c] < 0) | (bb[c] > 360)), c] = np.nan

            # KEEP everything needed by BOTH:
            # - GNSS / motion
            # - node maps
            keep_bb_cols = [c for c in [
                "T", "TimeStamp", "Line",
            ...
            bb = bb[keep_bb_cols].copy()

        dsr = dsr_df.copy() if dsr_df is not None else pd.DataFrame()
        if not dsr.empty:
            for ts_col in ("TimeStamp", "TimeStamp1"):
                if ts_col in dsr.columns:
                    dsr[ts_col] = pd.to_datetime(dsr[ts_col], errors="coerce")
                else:
                    dsr[ts_col] = pd.NaT
            ...
            for c in numeric_dsr_cols:
                if c in dsr.columns:
                    dsr[c] = pd.to_numeric(dsr[c], errors="coerce")
            ...
            # KEEP node-map coordinate columns too
            keep_dsr_cols = [c for c in [
                "Line", "Station", "TimeStamp", "TimeStamp1", "ROV", "ROV1",
                "PreplotEasting", "PreplotNorthing",
                "PrimaryEasting", "PrimaryNorthing",
                "SecondaryEasting", "SecondaryNorthing",
            ] if c in dsr.columns]
            ...
            dsr = (
                dsr[keep_dsr_cols]
                .dropna(subset=["TimeStamp", "TimeStamp1"], how="all")
                .sort_values(["TimeStamp", "TimeStamp1", "Station"], na_position="last")
    ...
    def _load_bbox_common_for_line(
            self,
            line,
            pad_minutes=0,
    ):
        ...
        common = self._prepare_bbox_common_dataset(
            bbdata=bbdata,
            dsr_df=dsr_df,
            cfg_row=cfg_row,
        )
        return common, start_time, end_time

    def get_bbox_common_dataset_for_line(
            self,
            line,
            pad_minutes=0,
    ):
        """
        Public getter for one shared prepared BBox dataset.

        Use this once, then pass `common=...` into:
            - plot_bbox_gnss_qc_for_line_paged()
            - plot_bbox_motion_qc_for_line_paged()
            - plot_bbox_motion_qc_for_line_paged_combined()
            - plot_whole_line_node_pages_fast()

        Returns:
            common, start_time, end_time
        """
        return self._load_bbox_common_for_line(
            line=line,
            pad_minutes=pad_minutes,
        )

    @staticmethod
    def _build_bbox_time_windows(tmin, tmax, hours_per_page):
        ...
        page_windows = []
        cur_start = pd.to_datetime(tmin).floor("min")
    ...
    @staticmethod
    def _filter_bbox_windows_with_node_activity(page_windows, dsr_df, activity_mode="both"):
        """Keep windows containing the requested DSR deployment/recovery timestamp."""
        if not page_windows or dsr_df is None or dsr_df.empty:
            return []

        column_lookup = {str(col).strip().lower(): col for col in dsr_df.columns}
        mode = str(activity_mode or "both").strip().lower()
        aliases = {
            "deployment": ("timestamp",),
            "recovery": ("timestamp1",),
        }.get(mode, ("timestamp", "timestamp1"))
        event_columns = []
        for alias in aliases:
            actual = column_lookup.get(alias)
            if actual is not None:
                event_columns.append(actual)

        if not event_columns:
            return []

        event_parts = [
            pd.to_datetime(dsr_df[col], errors="coerce").dropna()
        ...
        event_times = pd.concat(event_parts, ignore_index=True).dropna().sort_values().drop_duplicates()
        kept = []
        for window_start, window_end in page_windows:
            has_activity = ((event_times >= window_start) & (event_times < window_end)).any()
            if has_activity:
                kept.append((window_start, window_end))
        return kept

    @staticmethod
    def _resolve_bbox_max_workers(total_pages, max_workers=None):
        if total_pages <= 0:
            return 1
        if max_workers is not None:
            return max(1, min(int(max_workers), total_pages))
        cpu_n = os.cpu_count() or 2
        return max(1, min(cpu_n - 1, total_pages))
    ...
    def plot_bbox_gnss_qc_timeseries(
        ...
        Supports preprocessed common BBox dataset.
        """
        ...
        if not preprocessed:
            ...
            for c in ["GNSS1_NOS", "GNSS1_DiffAge", "GNSS1_HDOP", "GNSS2_NOS", "GNSS2_DiffAge", "GNSS2_HDOP"]:
                if c in df.columns:
                    df[c] = pd.to_numeric(df[c], errors="coerce")
                else:
                    df[c] = np.nan
            ...
            for c in ["GNSS1_DiffAge", "GNSS2_DiffAge"]:
                df.loc[(df[c].notna()) & ((df[c] < 0) | (df[c] > 9999)), c] = np.nan
            for c in ["GNSS1_HDOP", "GNSS2_HDOP"]:
        ...
        if dsr_df is not None and len(dsr_df) > 0:
            ...
            rov_values = sorted([x for x in dsr["ROV"].dropna().unique().tolist() if str(x).strip() != ""])
        ...
        ax = axes[0]
        ax.plot(df["T"], df.get("GNSS1_NOS", np.nan), linewidth=1.8, color="red",
                label=f"{g1} | {_stats_text(df.get('GNSS1_NOS', np.nan))}", zorder=3)
        ax.plot(df["T"], df.get("GNSS2_NOS", np.nan), linewidth=1.8, color="blue",
                label=f"{g2} | {_stats_text(df.get('GNSS2_NOS', np.nan))}", zorder=3)
        ...
        ax = axes[1]
        ax.plot(df["T"], df.get("GNSS1_DiffAge", np.nan), linewidth=1.2, color="red",
                label=f"{g1} | {_stats_text(df.get('GNSS1_DiffAge', np.nan))}", zorder=3)
        ax.plot(df["T"], df.get("GNSS2_DiffAge", np.nan), linewidth=1.2, color="blue",
                label=f"{g2} | {_stats_text(df.get('GNSS2_DiffAge', np.nan))}", zorder=3)
        ...
        self._style_ax(ax, ylabel="DiffAge, s", title=f"{title} — DiffAge (good≤{diff_good_max:g}, warn≤{diff_warn_max:g})")
        ax.grid(True, linestyle="--", alpha=0.30)
        ax.set_ylim(*diffage_ylim)
        ax.legend(loc="upper right", fontsize=8)

        ax = axes[2]
        ax.plot(df["T"], df.get("GNSS1_HDOP", np.nan), linewidth=1.8, color="red",
                label=f"{g1} | {_stats_text(df.get('GNSS1_HDOP', np.nan))}", zorder=3)
        ax.plot(df["T"], df.get("GNSS2_HDOP", np.nan), linewidth=1.8, color="blue",
                label=f"{g2} | {_stats_text(df.get('GNSS2_HDOP', np.nan))}", zorder=3)
        ...
        if station_events:
            for ax in axes:
                for ev in station_events:
                    ax.axvline(ev["t"], color=ev["color"], linewidth=1.2, alpha=0.6, linestyle="-", zorder=2)
        ...
        if rov_color_map:
            rov_handles = [Line2D([0], [0], color=color, lw=1.2, linestyle="--", label=str(rov))
                           for rov, color in rov_color_map.items()]
            fig.legend(handles=rov_handles, loc="upper left", bbox_to_anchor=(0.065, 0.985),
                       ncol=1, frameon=True, title="DSR Station Lines by ROV", fontsize=8, title_fontsize=9)
        ...
        if time_start is not None or time_end is not None:
            ...
            fig.suptitle(f"{title}\nPage window: {t1} -> {t2}", y=0.995, fontsize=12)

        fig.subplots_adjust(left=0.06, right=0.99, bottom=0.08, top=0.90, hspace=0.18)

        out_path = self._build_output_path(dsr_df if dsr_df is not None else bbdata, save_dir, suffix, file_name, ext)
        self._save_figure(fig, out_path, dpi=dpi, close=False)
    ...
    def plot_bbox_gnss_qc_for_line_paged(
        ...
        Reuses one common prepared dataset for all pages.
        Can render pages in parallel with multiprocessing.
        """

        if common is None:
            common, start_time, end_time = self.get_bbox_common_dataset_for_line(
                line=line,
                pad_minutes=pad_minutes,
            )
        else:
            bb_tmp = common.get("bbdata") if isinstance(common, dict) else None
            if bb_tmp is not None and len(bb_tmp) > 0 and "T" in bb_tmp.columns:
                start_time = bb_tmp["T"].min()
                end_time = bb_tmp["T"].max()
            else:
                start_time = None
                end_time = None
        ...
        page_windows = self._build_bbox_time_windows(tmin, tmax, hours_per_page)
        page_windows = self._filter_bbox_windows_with_node_activity(page_windows, dsr_df, activity_mode)
        total_pages = len(page_windows)
        if total_pages == 0:
            return []

        if use_multiprocess and total_pages > 1 and str(activity_mode).lower() == "both":
            workers = self._resolve_bbox_max_workers(total_pages, max_workers=max_workers)
            jobs = []
            for page_no, (cur_start, cur_end) in enumerate(page_windows, start=1):
                ...
                jobs.append((
                    str(self.db_path), common, line, page_no, total_pages,
                    cur_start, cur_end, save_dir, fname, ext, dpi, figsize, bb_stride
                ))

            saved_paths = [None] * total_pages
            ctx = mp.get_context("spawn")
            with ProcessPoolExecutor(max_workers=workers, mp_context=ctx) as ex:
                futures = [ex.submit(_bbox_gnss_qc_page_worker, job) for job in jobs]
                for fut in as_completed(futures):
                    page_no, out_path = fut.result()
                    saved_paths[page_no - 1] = out_path
            return saved_paths

        activity_label = {"deployment": "Deployment", "recovery": "Recovery"}.get(
            str(activity_mode).lower(), ""
        )
        title_scope = f"{activity_label} " if activity_label else ""
        saved_paths = []
        for page_no, (cur_start, cur_end) in enumerate(page_windows, start=1):
            if file_prefix:
                fname = f"{file_prefix}_page_{page_no:03d}.{ext.lstrip('.')}"
            ...
            _, out_path = self.plot_bbox_gnss_qc_timeseries(
                bbdata=bb,
                dsr_df=dsr_df,
                title=f"Line {line} {title_scope}GNSS QC | Page {page_no} of {total_pages}",
                gnss1_label=common.get("gnss1_label"),
                gnss2_label=common.get("gnss2_label"),
                time_start=cur_start,
                time_end=cur_end,
    ...
    def plot_bbox_motion_qc_timeseries(
        ...
        if not preprocessed:
            if metric == "SOG":
                for c in [vessel_col, rov1_col, rov2_col]:
                    df.loc[(df[c].notna()) & ((df[c] < 0) | (df[c] > 25)), c] = np.nan
            else:
                for c in [vessel_col, rov1_col, rov2_col]:
                    df.loc[(df[c].notna()) & ((df[c] < 0) | (df[c] > 360)), c] = np.nan
        ...
        def _break_angle_wrap(series, threshold=180.0):
            ...
            for i in range(1, len(s)):
                a = s.iloc[i - 1]
                b = s.iloc[i]
                if pd.notna(a) and pd.notna(b) and abs(float(b) - float(a)) > threshold:
                    s.iloc[i] = np.nan
            return s
        ...
        for ax, col, lbl, color in plot_defs:
            ...
            ax.plot(df["T"], y, linewidth=line_width, color=color, label=f"{lbl} | {_stats_text(df[col])}", zorder=3)

            ylabel = "SOG, kn" if metric == "SOG" else f"{metric}, deg"
            ttl = f"{title} — {lbl} {metric}"
            ...
            for i, ev in enumerate(station_events):
                ...
                if pd.notna(st):
                    try:
                        ...
                        st_txt = str(st)
                    axb.text(ev["t"], ytxt, st_txt, rotation=station_label_rotation,
                             fontsize=station_label_fontsize, fontweight="bold",
                             color=ev["color"], ha="center", va="bottom",
                             alpha=station_label_alpha, zorder=5,
                             bbox=dict(facecolor="white", edgecolor="none", alpha=0.7, pad=0.5))
    ...
    def plot_bbox_motion_qc_for_line_paged(
        ...
        metric = str(metric).strip().upper()
        page_windows = self._build_bbox_time_windows(tmin, tmax, hours_per_page)
        page_windows = self._filter_bbox_windows_with_node_activity(page_windows, dsr_df)
        total_pages = len(page_windows)
        if total_pages == 0:
            return []

        if use_multiprocess and total_pages > 1:
            workers = self._resolve_bbox_max_workers(total_pages, max_workers=max_workers)
            jobs = []
            for page_no, (cur_start, cur_end) in enumerate(page_windows, start=1):
                if file_prefix:
                ...
                jobs.append((
                    str(self.db_path), common, line, metric, page_no, total_pages,
                    cur_start, cur_end, save_dir, fname, ext, dpi, figsize, bb_stride, y_lim
                ))

            saved_paths = [None] * total_pages
            ctx = mp.get_context("spawn")
            with ProcessPoolExecutor(max_workers=workers, mp_context=ctx) as ex:
                futures = [ex.submit(_bbox_motion_metric_page_worker, job) for job in jobs]
                for fut in as_completed(futures):
                    page_no, out_path = fut.result()
                    saved_paths[page_no - 1] = out_path
            return saved_paths
    ...
    def plot_bbox_motion_qc_combined_timeseries(
        ...
        Optimized for reusing one common prepared dataset.
        """
        ...
        sog_v = _num("VesselSOG").where(lambda s: (s >= 0) & (s <= 5), np.nan)
        sog_r1 = _num("ROV1_SOG").where(lambda s: (s >= 0) & (s <= 5), np.nan)
        sog_r2 = _num("ROV2_SOG").where(lambda s: (s >= 0) & (s <= 5), np.nan)
        ...
        if dsr_df is not None and len(dsr_df) > 0:
            ...
            for ax in axes:
                for _, r in dsr.iterrows():
                    ax.axvline(r["TimeStamp"], color=rov_color_map.get(r["ROV"], "#999999"),
                               lw=1.2, alpha=0.60, linestyle="-", zorder=2)
            ...
            for i, (_, r) in enumerate(dsr.iterrows()):
                ...
                if pd.notna(st):
                    try:
                        ...
                        st_txt = str(st)
                    axb.text(r["TimeStamp"], ytxt, st_txt, rotation=station_label_rotation,
                             fontsize=station_fontsize, color=rov_color_map.get(r["ROV"], "black"),
                             ha="center", va="bottom", alpha=station_label_alpha, zorder=5,
        ...
        fig.subplots_adjust(left=0.06, right=0.99, top=0.84, bottom=0.08, hspace=0.16)
    ...
    def plot_bbox_motion_qc_for_line_paged_combined(
        ...
        """
        Build paged combined motion QC plots for one DSR line.
        Reuses one common prepared dataset for all pages.
        Can render pages in parallel with multiprocessing.
        """
        ...
        if use_multiprocess and total_pages > 1 and str(activity_mode).lower() == "both":
            ...
            ctx = mp.get_context("spawn")
            with ProcessPoolExecutor(max_workers=workers, mp_context=ctx) as ex:
                futures = [ex.submit(_bbox_motion_combined_page_worker, job) for job in jobs]
                for fut in as_completed(futures):
                    page_no, out_path = fut.result()
                    saved_paths[page_no - 1] = out_path
            return saved_paths
        ...
        for page_no, (cur_start, cur_end) in enumerate(page_windows, start=1):
            ...
            _, out_path = self.plot_bbox_motion_qc_combined_timeseries(
                bbdata=bb,
                dsr_df=dsr_df,
                title=f"Line {line} {title_scope}Motion QC | Page {page_no} of {total_pages}",
                vessel_label=common.get("vessel_label"),
                rov1_label=common.get("rov1_label"),
                rov2_label=common.get("rov2_label"),
                time_start=cur_start,
    ...
    def plot_bbox_position_difference_qc_timeseries(
            self,
            bbdata,
            dsr_df=None,
            *,
            title="POSITION DIFFERENCE QC",
            time_start=None,
            time_end=None,
            bb_stride=20,
            figsize=(18, 10),
            save_dir=".",
            file_name=None,
            suffix="bbox_position_difference_qc",
            ext="png",
            dpi=200,
            is_show=False,
            close=True,
        ...
        """Plot GNSS1-GNSS2 and ROV1 INS-USBL Easting/Northing differences."""
        if preprocessed:
            df = bbdata.copy()
        else:
            common = self._prepare_bbox_common_dataset(bbdata, dsr_df=dsr_df)
            df = common["bbdata"]
            dsr_df = common["dsr_df"]

        if df is None or df.empty:
            return self.empty_figure(title=title, message="No BlackBox data"), None

        if time_start is not None:
            df = df.loc[df["T"] >= pd.to_datetime(time_start)]
        if time_end is not None:
            df = df.loc[df["T"] < pd.to_datetime(time_end)]
        if df.empty:
            return self.empty_figure(title=title, message="No BlackBox data in page window"), None

        stride = max(int(bb_stride), 1)
        work = df.iloc[::stride].copy()
        gnss1_name = str(gnss1_label or "GNSS1")
        gnss2_name = str(gnss2_label or "GNSS2")
        difference_defs = [
            ("GNSS1_Easting", "GNSS2_Easting", f"{gnss1_name} Easting - {gnss2_name} Easting", "#1f77b4"),
            ("GNSS1_Northing", "GNSS2_Northing", f"{gnss1_name} Northing - {gnss2_name} Northing", "#ff7f0e"),
            ("ROV1_INS_Easting", "ROV1_USBL_Easting", "ROV1 Easting INS - USBL", "#2ca02c"),
            ("ROV1_INS_Northing", "ROV1_USBL_Northing", "ROV1 Northing INS - USBL", "#9467bd"),
        ]

        fig, axes = plt.subplots(4, 1, sharex=True, figsize=figsize)
        for ax, (col_a, col_b, panel_title, color) in zip(axes, difference_defs):
            if col_a in work.columns and col_b in work.columns:
                values = pd.to_numeric(work[col_a], errors="coerce") - pd.to_numeric(work[col_b], errors="coerce")
                finite = values.dropna()
                if finite.empty:
                    ax.text(0.5, 0.5, "No valid coordinate data", transform=ax.transAxes,
                            ha="center", va="center", color="#666666")
                else:
                    stats = f"min:{finite.min():.2f}; max:{finite.max():.2f}; avg:{finite.mean():.2f} m"
                    ax.plot(work["T"], values, color=color, linewidth=1.25, label=stats)
                    ax.legend(loc="upper right", fontsize=8, frameon=True)
            else:
                ax.text(0.5, 0.5, f"Columns unavailable: {col_a}, {col_b}",
                        transform=ax.transAxes, ha="center", va="center", color="#666666")
            ax.axhline(0.0, color="black", linestyle="--", linewidth=0.75, alpha=0.75)
            ax.set_title(panel_title, fontsize=11, fontweight="bold")
            ax.set_ylabel("Difference (m)")
            ax.grid(True, linewidth=0.35, alpha=0.55)

        # Mark all deployment and recovery events occurring in this window.
        if dsr_df is not None and not dsr_df.empty:
            for ts_col in ("TimeStamp", "TimeStamp1"):
                if ts_col not in dsr_df.columns:
                    continue
                event_times = pd.to_datetime(dsr_df[ts_col], errors="coerce").dropna()
                if time_start is not None:
                    event_times = event_times[event_times >= pd.to_datetime(time_start)]
                if time_end is not None:
                    event_times = event_times[event_times < pd.to_datetime(time_end)]
                for event_time in event_times:
                    for ax in axes:
                        ax.axvline(event_time, color="#7f8c8d", linewidth=0.35, alpha=0.24, zorder=0)

        axes[-1].set_xlabel("Time")
        window_text = ""
        if time_start is not None and time_end is not None:
            window_text = f"\nPage window: {pd.to_datetime(time_start):%Y-%m-%d %H:%M} -> {pd.to_datetime(time_end):%Y-%m-%d %H:%M}"
        fig.suptitle(f"{title}{window_text}", fontsize=14, y=0.985)
        fig.subplots_adjust(left=0.065, right=0.985, top=0.91, bottom=0.075, hspace=0.42)

        if file_name:
            out_path = Path(save_dir) / file_name
        else:
            out_path = Path(save_dir) / f"{suffix}.{ext.lstrip('.')}"
    ...
    def plot_bbox_position_difference_qc_for_line_paged(
            self,
            line,
            *,
            common=None,
        ...
        """Build activity-filtered 24-hour position-difference QC pages."""
        if common is None:
            common, _start_time, _end_time = self.get_bbox_common_dataset_for_line(
                line=line, pad_minutes=pad_minutes
            )
        bb = common["bbdata"]
        dsr_df = common["dsr_df"]
        if bb is None or bb.empty:
        ...
        page_windows = self._build_bbox_time_windows(bb["T"].min(), bb["T"].max(), hours_per_page)
        page_windows = self._filter_bbox_windows_with_node_activity(page_windows, dsr_df, activity_mode)
        total_pages = len(page_windows)
        activity_label = {"deployment": "Deployment", "recovery": "Recovery"}.get(
            str(activity_mode).lower(), ""
        ...
        saved_paths = []
        for page_no, (cur_start, cur_end) in enumerate(page_windows, start=1):
            prefix = file_prefix or f"{line}_bbox_position_difference_qc"
            fname = f"{prefix}_page_{page_no:03d}.{ext.lstrip('.')}"
            _, out_path = self.plot_bbox_position_difference_qc_timeseries(
                bbdata=bb,
                dsr_df=dsr_df,
                title=f"Line {line} {title_scope}Position Difference QC | Page {page_no} of {total_pages}",
                time_start=cur_start,
                time_end=cur_end,
                bb_stride=bb_stride,
                figsize=figsize,
    ...
    def plot_bbox_mru_difference_qc_timeseries(
            self,
            bbdata,
            dsr_df=None,
            *,
            title="VESSEL MRU1 VS MRU2 QC",
            time_start=None,
            time_end=None,
            bb_stride=20,
            figsize=(18, 10),
        ...
        """Plot Vessel MRU1-MRU2 heading, pitch, and roll differences."""
        if preprocessed:
            df = bbdata.copy()
        else:
            common = self._prepare_bbox_common_dataset(bbdata, dsr_df=dsr_df)
        ...
        work = df.iloc[::max(int(bb_stride), 1)].copy()
        panels = [
            ("HDG", "Heading difference MRU1 - MRU2", "#1f77b4", True),
            ("PITCH", "Pitch difference MRU1 - MRU2", "#2ca02c", False),
            ("ROLL", "Roll difference MRU1 - MRU2", "#d62728", False),
        ]
        fig, axes = plt.subplots(3, 1, sharex=True, figsize=figsize)
        for ax, (metric, panel_title, color, circular) in zip(axes, panels):
            col_1 = f"Vessel_MRU1_{metric}"
            col_2 = f"Vessel_MRU2_{metric}"
            if col_1 in work.columns and col_2 in work.columns:
                values = pd.to_numeric(work[col_1], errors="coerce") - pd.to_numeric(work[col_2], errors="coerce")
                if circular:
                    values = ((values + 180.0) % 360.0) - 180.0
                finite = values.dropna()
                if finite.empty:
                    ax.text(0.5, 0.5, "No valid MRU data", transform=ax.transAxes,
                            ha="center", va="center", color="#666666")
                else:
                    stats = f"min:{finite.min():.2f}; max:{finite.max():.2f}; avg:{finite.mean():.2f} deg"
                    ax.plot(work["T"], values, color=color, linewidth=1.25, label=stats)
                    ax.legend(loc="upper right", fontsize=8, frameon=True)
            else:
                ax.text(0.5, 0.5, f"Columns unavailable: {col_1}, {col_2}",
                        transform=ax.transAxes, ha="center", va="center", color="#666666")
            ax.axhline(0.0, color="black", linestyle="--", linewidth=0.75, alpha=0.75)
            ax.set_title(panel_title, fontsize=11, fontweight="bold")
        ...
        if time_start is not None and time_end is not None:
            window_text = f"\nPage window: {pd.to_datetime(time_start):%Y-%m-%d %H:%M} -> {pd.to_datetime(time_end):%Y-%m-%d %H:%M}"
        fig.suptitle(f"{title}{window_text}", fontsize=14, y=0.985)
        fig.subplots_adjust(left=0.065, right=0.985, top=0.90, bottom=0.075, hspace=0.42)

        out_path = Path(save_dir) / (file_name or f"{suffix}.{ext.lstrip('.')}")
        out_path.parent.mkdir(parents=True, exist_ok=True)
        self._save_figure(fig, out_path, dpi=dpi, close=False)
        if is_show:
            plt.show()
    ...
    def plot_bbox_mru_difference_qc_for_line_paged(
            self,
            line,
            *,
            common=None,
        ...
        """Build activity-filtered 24-hour Vessel MRU1-vs-MRU2 QC pages."""
        if common is None:
            common, _start_time, _end_time = self.get_bbox_common_dataset_for_line(
                line=line, pad_minutes=pad_minutes
            )
        ...
        saved_paths = []
        for page_no, (cur_start, cur_end) in enumerate(page_windows, start=1):
            prefix = file_prefix or f"{line}_bbox_mru_difference_qc"
            fname = f"{prefix}_page_{page_no:03d}.{ext.lstrip('.')}"
            _, out_path = self.plot_bbox_mru_difference_qc_timeseries(
                bbdata=bb,
                dsr_df=dsr_df,
                title=f"Line {line} {title_scope}Vessel MRU1 vs MRU2 QC | Page {page_no} of {total_pages}",
                time_start=cur_start,
                time_end=cur_end,
                bb_stride=bb_stride,
                figsize=figsize,
    ...
    def plot_bbox_gnss_and_motion_qc_for_line_paged(
            self,
            line,
            *,
            hours_per_page=24,
            pad_minutes=0,
            save_dir=".",
            gnss_file_prefix=None,
            motion_file_prefix=None,
            position_difference_file_prefix=None,
            mru_difference_file_prefix=None,
            ext="png",
            dpi=200,
            figsize=(18, 10),
            is_show=False,
        ...
        """
        Load one shared prepared BBox dataset once and use it for both:
            - plot_bbox_gnss_qc_for_line_paged()
            - plot_bbox_motion_qc_for_line_paged_combined()
        ...
        gnss_pages, motion_pages = [], []
        position_difference_pages, mru_difference_pages = [], []

        # Keep deployment and recovery QC as distinct page groups. DSR.TimeStamp
        # selects deployment pages; DSR.TimeStamp1 selects recovery pages.
        for activity_mode in ("deployment", "recovery"):
            def scoped_prefix(prefix, default_suffix):
                base = prefix or f"{line}_{default_suffix}"
                return f"{base}_{activity_mode}"

            gnss_pages.extend(self.plot_bbox_gnss_qc_for_line_paged(
                line=line, common=common, hours_per_page=hours_per_page,
                pad_minutes=pad_minutes, save_dir=save_dir,
                file_prefix=scoped_prefix(gnss_file_prefix, "bbox_gnss_qc"),
                ext=ext, dpi=dpi, figsize=figsize, is_show=is_show, close=close,
                bb_stride=bb_stride, use_multiprocess=use_multiprocess,
                max_workers=max_workers, activity_mode=activity_mode,
            ))
            motion_pages.extend(self.plot_bbox_motion_qc_for_line_paged_combined(
                line=line, common=common, hours_per_page=hours_per_page,
                pad_minutes=pad_minutes, save_dir=save_dir,
                file_prefix=scoped_prefix(motion_file_prefix, "bbox_motion_qc"),
                ext=ext, dpi=dpi, figsize=figsize, is_show=is_show, close=close,
                bb_stride=bb_stride, use_multiprocess=use_multiprocess,
                max_workers=max_workers, activity_mode=activity_mode,
            ))
            position_difference_pages.extend(self.plot_bbox_position_difference_qc_for_line_paged(
                line=line, common=common, hours_per_page=hours_per_page,
                pad_minutes=pad_minutes, save_dir=save_dir,
                file_prefix=scoped_prefix(position_difference_file_prefix, "bbox_position_difference_qc"),
                ext=ext, dpi=dpi, figsize=figsize, is_show=is_show, close=close,
                bb_stride=bb_stride, activity_mode=activity_mode,
            ))
            mru_difference_pages.extend(self.plot_bbox_mru_difference_qc_for_line_paged(
                line=line, common=common, hours_per_page=hours_per_page,
                pad_minutes=pad_minutes, save_dir=save_dir,
                file_prefix=scoped_prefix(mru_difference_file_prefix, "bbox_mru_difference_qc"),
                ext=ext, dpi=dpi, figsize=figsize, is_show=is_show, close=close,
                bb_stride=bb_stride, activity_mode=activity_mode,
            ))
        ...
        return {
            "common": common,
            "gnss_pages": gnss_pages,
            "motion_pages": motion_pages,
            "position_difference_pages": position_difference_pages,
            "mru_difference_pages": mru_difference_pages,
        }
