...
function ensureSelectedOption(selectEl, value) {
  ...
  const has = Array.from(selectEl.options).some(
    (o) => normalizeKey(o.value) === v
  );
  ...
  selectEl.value = found ? found.value : v;
}

function applyMappingToSelectors(mapping = null) {
  const map = mapping || window.__bboxCurrentMapping || {};
...
function setAlert(msgWrap, msgEl, type, text) {
  ...
  if (csvErr) {
    csvErr.className = `alert alert-${type} mt-3`;
    csvErr.textContent = text;
    csvErr.classList.remove("d-none");
  }
}

function clearAlert(msgWrap) {
  if (msgWrap) msgWrap.classList.add("d-none");

  const csvErr = document.getElementById("csvErr");
  if (csvErr) {
    csvErr.textContent = "";
    csvErr.classList.add("d-none");
  }
}

function setSelectorsOptions(optionsHtml) {
  document.querySelectorAll(".bbox-config-selector").forEach((sel) => {
    ...
    if (currentValue) {
      ensureSelectedOption(sel, currentValue);
    }
  });
}
...
window.__bboxCurrentMapping = {};
window.__bboxApplyMappingToSelectors = applyMappingToSelectors;
window.__bboxCollectMappingFromUI = collectMappingFromUI;
window.__bboxSetSelectorsOptions = setSelectorsOptions;
...
export function initBBoxConfigDatalist() {
  ...
  const deleteTpl = nameInput.dataset.configDeleteUrlTemplate;
  const importUrl = nameInput.dataset.configImportUrl;
  const exportAllUrl = nameInput.dataset.configExportAllUrl;
  ...
  async function fetchDetailById(id) {
    ...
    if (!res.ok || !data.ok) {
      throw new Error(data.error || "Failed to load config detail");
    }
  ...
  function applyConfigToUI(configObj, mappingObj) {
    ...
    window.__bboxCurrentMapping = mappingObj || {};
    applyMappingToSelectors(window.__bboxCurrentMapping);
  }
  ...
  // -----------------------------------------
  // case 1: full exported DB JSON
  // -----------------------------------------
  if (
    obj &&
    Array.isArray(obj.BBox_Configs_List) &&
    Array.isArray(obj.BBox_Config)
  ) {
    const res = await fetch(importUrl, {
      method: "POST",
      headers: {
    ...
    setAlert(
      ...
      msgEl,
      "success",
      `Imported ${data.imported_parents || 0} configs and ${data.imported_children || 0} fields.`
    );
    return;
  }

  // -----------------------------------------
  // case 2: single config JSON
  // -----------------------------------------
  ...
  if (!r.ok) {
    setAlert(msgWrap, msgEl, "warning", r.error || "Imported to UI only.");
    return;
  }
  ...
  if (btnDelete) {
    btnDelete.addEventListener("click", () => {
      deleteCurrentConfig().catch((e) => {
        console.error(e);
        setAlert(msgWrap, msgEl, "danger", e.message || "Delete failed.");
      });
    });
  }
  ...
  if (btnExport) {
    btnExport.addEventListener("click", () => {
      exportAllConfigsJson().catch((e) => {
        console.error(e);
        setAlert(msgWrap, msgEl, "danger", e.message || "Export failed.");
      });
    });
  }
  ...
  loadConfigs()
    .then(() => {
      if (nameInput.value) {
        return loadAndApplyConfigByName(nameInput.value);
      }
    })
    .catch((e) => {
      console.error(e);
      setAlert(
        ...
        msgEl,
        "danger",
        e.message || "Failed to initialize BBox configs."
      );
    })
    .finally(() => {
      setDeleteState();
