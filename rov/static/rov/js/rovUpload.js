...
export function initRovUploadModal() {
    ...
    const rules = {
        DSR: {
            accept: ".txt,.csv",
            help: "DSR files (.txt / .csv)",
            url: "urlDsr",
        },
        SURVEY_MANAGER: {
            accept: ".csv,.txt",
            help: "Survey Manager (.csv / .txt)",
            url: "urlSurveyManager",
        },
        BLACK_BOX: {
            accept: ".csv,.txt",
            help: "Black Box logs (.csv / .txt)",
            url: "urlBlackBox",
        },
        REC_DB: {
            accept: ".*,.txt",
    ...
    function setCfgVisibility(fileType) {
        ...
        if (cfgGroup) {
            cfgGroup.classList.toggle("d-none", !show);
        }
    }
...
Processed: ${data.total_processed ?? 0}, upserted: ${data.total_upserted ?? 0}, skipped: ${data.total_skipped ?? 0}.`;
        }

        return "Upload completed successfully.";
    }

    function updateHtmlTargets(data) {
        if (data.bbox_file_tbody !== undefined && tbody) {
            tbody.innerHTML = data.bbox_file_tbody;
        }

        // DSR / SM use dsr_lines_body.
        // REC_DB currently returns dsr_line_body_html.
        const dsrHtml = data.dsr_lines_body || data.dsr_line_body_html;
        if (dsrHtml && dsrbody) {
            dsrbody.innerHTML = dsrHtml;
        }
    ...
    async function refreshProgressMap() {
        const wrap = document.getElementById("rov-progress-map-wrap");
        const target = document.getElementById("rov-progress-map-target");

        if (!wrap || !target) return;

        const url = wrap.dataset.progressMapUrl;
        if (!url) return;

        target.innerHTML = `
            <div class="d-flex justify-content-center align-items-center p-4 text-muted">
                <span class="spinner-border spinner-border-sm me-2"></span>
                Refreshing progress map...
            </div>
        `;
        ...
        if (!resp.ok || !data.ok) {
            throw new Error(data.error || "Progress map refresh failed");
        }

        target.innerHTML = "";

        if (window.Bokeh && data.item) {
            window.Bokeh.embed.embed_item(data.item);
        }
    }
    ...
    form.addEventListener("submit", async (e) => {
        ...
        try {
            ...
            updateHtmlTargets(data);

            // Redraw progress map after data that changes progress layers.
            if (["DSR", "SURVEY_MANAGER", "REC_DB"].includes(fileType)) {
                setStatus("Refreshing progress map...");
