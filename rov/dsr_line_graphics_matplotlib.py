...
class DSRLineGraphicsMatplotlib:
    ...
    def _prepare_bbox_common_dataset(
        ...
        This is intended to be reused by paged GNSS / motion plots.
        """
        ...
        if not dsr.empty:
            ...
            keep_dsr_cols = [c for c in ["Line", "Station", "TimeStamp", "ROV"] if c in dsr.columns]
            dsr = dsr[keep_dsr_cols].dropna(subset=["TimeStamp"]).sort_values(["TimeStamp", "Station"]).reset_index(drop=True)
